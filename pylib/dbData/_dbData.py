...
from _mask1D import Mask1D
from _mask2D import Mask2D, mask2d_op
from _plotdbData import PlotdbData
# from pylib.etMisc import loading_bar
from _showmask import ShowMask
...
class plot_properties(object):
    ...
    def __init__(self, prod):
        ...
        self.props["contourcolor"] = "nipy_spectral"
...
class DataDict(dict):
    ...
    def mask(self, masks):
        ...
        if isinstance(masks, np.ndarray):
            ...
            applied_mask = np.ones(self.__getitem__(self.keys()[0]).shape, dtype=bool)
            for mask in masks:
                applied_mask *= mask
...
class dbData(object):
    ...
    def get_all_data(self, masks=[]):
        ...
        if isinstance(masks, (str, np.ndarray)):
            masks = [masks]
        if masks != []:
            applied_mask = np.ones(self.data[self.data.keys()[0]].shape, dtype=bool)
            for mask in masks:
    ...
    def get_data(self, masks, prods):
        ...
        applied_mask = np.ones(
            self.data[self.data.keys()[0]].shape[0], dtype=bool)
        for mask in masks:
            if isinstance(mask, np.ndarray):
                applied_mask *= mask
            else:
    ...
    def save_subset(self, mask, filename="tmp.dat", prods="all"):
        ...
        if prods == "all":
            ...
            for prod in prods:
                subset[prod] = self.data[prod][self.get_mask(mask)]
        cPickle.dump(subset, open(filename, "w"), protocol=2)
            ...
            def _add_directmasks(key):
                if load_direct_masks and mask_dict[key].directmasks:
                    ...
                    for key2 in mask_dict[key].directmasks:
                        if mask_dict[key].directmasks[key2].shape[0] == self.data[self.data.keys()[0]].shape[0]:
...
def generatekmeansmasks(data, k, name="Default", prodlist=[]):
    ...
    for i in range(0, k):
        masks[name + ' ' + str(i + 1)] = np.zeros(
            data[data.keys()[0]].shape[0], dtype=bool)
