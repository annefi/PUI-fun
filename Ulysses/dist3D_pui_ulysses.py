...
class Dist3D(object):
    def __init__(self, d, mass=4., charge=1., aspphistep=1., aspthetastep=1., nrs_perp=1, nrs_para=3, nrs_sec=1,
        ...
        # nrs_epq -> deltaEpQ is +-3% -> delta v is +-1.5% -> nrs_epq equal spaced velocities in epq direction are
    ...
    def _calc_wspace(self, ):
        ...
        """
        self.w3dspace = zeros(
            (self.vswbins.shape[0], self.aspphi.shape[0], self.asptheta.shape[0], 60, 3, 8, 3, self.sec_det_dim))
        self.wspace = zeros(
            (self.vswbins.shape[0], self.aspphi.shape[0], self.asptheta.shape[0], 60, 3, 8, 1, self.sec_det_dim))
    ...
    def _add_w(self):
        ...
        if not "wHe1+2" in self.d.data.keys():
            self.d.add_data("wHe1+2",
                            getvelocity_allsteps(4., 1., self.d.data["epq"].astype(int)) / around(self.d.data["vsw"],
                                                                                                  -1))
        else:
            self.d.data["wHe1+2"] = getvelocity_allsteps(4., 1., self.d.data["epq"].astype(int)) / around(
                self.d.data["vsw"], -1)
    ...
    def plot_wspec(self, cut_x = 10, ax = None):
        wbins = arange(-2,2.01,0.2)
        norm_arr, H = self.calc_w3dspecs()
        norm_arr[norm_arr == 0] = 1
        if ax == None:
            fig, ax = plt.subplots()
            colormap = plt.cm.get_cmap("viridis")
            self.Quadmesh = ax.pcolormesh(wbins,wbins,H[cut_x,:,:], cmap=colormap)
