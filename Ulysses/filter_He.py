...
class Filter_He(uswipha):
    """Write and save a subset (e.g. for one ion species) of Ulysses PHA words.
    ...
    def filter_via_mask(self,esa,tch,species = 'He1+'):
        '''
        Determine the outer edges of a "species tube" in an esa-tch matrix and write them to a file.

        esa and tch are the resp. data of a mask2D that is drawn manually around the species (e.g. He+) in the ESA vs. Tch matrix.
        Pass the arguments as "esa = d.get_data('maskname','epq')", "tch = ...".
    ...
    def filter_ech(self,tch,ech,species = 'He1+'):
