...
__host__ __device__
#endif
	MatrixNxN<dim, T> &operator=(const Matrix<dim, dim, T> &other);
		/*!< \brief assignment operator															*/
...
/*
template<uint dim, class T>
MatrixNxN<dim, T> &MatrixNxN<dim, T>::operator=(const Matrix<dim, dim, T> &other)
{
    if(this == &other)
        return *this;
...
double MatrixNxN<dim, T>::det(){
    ...
    if(dim == 2)
        return((double)this->content[0] * (double)this->content[3] - (double)this->content[2] * (double)this->content[1]);
    else
    {
...
__host__ __device__
#endif
    void convertSphericalToCartesian(hcFloat x, hcFloat y, hcFloat z);//*/
...
__host__ __device__
#endif
    void loadEulerTransform(S omega, S theta, S phi);	// = Rz(phi)*Rx(theta)*Rz(omega) (see Heliospheric coordinate systems, Fränz, Harper 2002)
