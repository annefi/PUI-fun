...
class collimator(object):
    ...
    def test_plot(self):
        ...
        # x-axis:
        ax.plot([0,3],[0,0],[0,0], '-', color="k", lw=.5)
        # y-axis:
        ax.plot([0, 0], [0, 3], [0, 0], '-', color="k", lw=.5)
        # z-axis:
        ax.plot([0, 0], [0, 0], [0, 3], '-', color="k", lw=.5)
        ...
        ax.plot([0, self.rphiax[0]*2.5], [0, self.rphiax[1]*2.5], [0, self.rphiax[2]*2.5], "-", color="royalblue",
                ls=":")
        ...
        ax.plot([0, self.rax[0]], [0, self.rax[1]], [0, self.rax[2]], "-", color="green")
    ...
    def _calc_points(self):
        ...
        rax is the axis from which the entrance slit can be constructed by rotating the vector (1,0,0) by ang_para
        (full 90 degrees, resulting in an opening angle of the collimator of ~70 deg against the x-y plane).
        Each detector is assumed to cover 30 deg. The opening in the x-y plane is +-2 deg.
        det1,2,3 are a representation of the collimator in this coordinate system.
        It has to be rotated to the actual scientific cordinates.
        This is done later on by, first rotate to the actual spin axis direction (_rot_aspang) ,
        then to the direction where the rotation is started,i.e. the sun-pulse is triggered (_rot_seczero).
        """
    ...
    def _rot_aspang(self):
        ...
        Coordinates GSE(basically SpacecraftSunEcliptic
        x-> Spacecraft-Sun,y -> Against Earth(Spacecraft) Orbit in ecliptic. z -> Out of ecliptic (North)
        SpaceCraft-Spin-Axis is then defined by first rotate (1,0,0) by in ecliptic aspect angle (phi) around (0,0,1)
        (for ACE-SWICS for Ulysses-SWICS it might be different and Axis might be different).
        Then the vector is rotated out of the ecliptic by rotating around the also rotated y-Axis by the out of ecliptic aspect angle. T
        his angle is right now defined as 0 in ecliptic positve angle north ,i.e above the ecliptic.
        Because definition is against the typical sense of rotation in an right-handed system,
        the rotation actually has to be done by the negative angle.
        """
