...
class Dist3D(object):
    def __init__(self,d,mass = 4., charge = 1., aspphistep=1.,aspthetastep=1.,vswbins=arange(300.,800.1,10.),ion="He1+"):
        ...
        self.vels = getvelocity_allsteps(self.mass,self.charge,arange(0,60,1,))
    ...
    def _calc_vspace(self):
        ...
        """
        self.vspace = zeros((self.aspphi.shape[0],self.asptheta.shape[0],60,3,8,3,1))
    ...
    def _add_3Dv(self):
        ...
        """
        phiind = searchsorted(self.aspphi,around(self.d.data["aspphi"]))
        thetaind = searchsorted(self.asptheta,around(self.d.data["asptheta"]))
        epqind = self.d.data["epq"].astype(int)
        detind = self.d.data["did"].astype(int)
        # set DCR to det 0 
        detind[detind==7] = 0
    ...
    def _calc_phspeff_wgt(self):
        """
        Loads ion efficiencies ion name, mass and charge must be given at the initialisation of the class object
        """
