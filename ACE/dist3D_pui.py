from ACE.SWICS.tools.swics_collimator import collimator
...
class Dist3D(object):
    def __init__(self,d,mass = 4., charge = 1., aspphistep=1.,aspthetastep=1.,nrs_perp=1,nrs_para=3,nrs_sec=3,nrs_epq=1,vswbins=arange(300.,800.1,10.),ion="He1+"):
        """
        ...
        self.vels = getvelocity_allsteps(self.mass,self.charge,arange(0,60,1,),frac=1.)
        # nrs_epq -> deltaEpQ is +-3% -> delta v is +-1.5% -> nrs_epq equal spaced velocities in epq direction are generated by vels_fac
        self.vels_fac = linspace(0.985+0.015/self.nrs_epq,1.015-0.015/self.nrs_epq,self.nrs_epq)
    ...
    def _calc_FoV(self):
        ...
        for iphi,phi in enumerate(self.aspphi):
            for itheta,theta in enumerate(self.asptheta):
                c = collimator(aspphi=phi,asptheta=theta,nrs_perp=self.nrs_perp,nrs_para=self.nrs_para,nrs_sec=self.nrs_sec)
    ...
    def _calc_wspace(self,):
        ...
        Calculates vx,vy,vz for all epqsteps and given aspect angles
        """
        self.w3dspace = zeros((self.vswbins.shape[0],self.aspphi.shape[0],self.asptheta.shape[0],60,3,8,3,self.sec_det_dim))
        #self.w3dphspace = zeros((self.vswbins.shape[0],self.aspphi.shape[0],self.asptheta.shape[0],60,3,8,3,self.dim))
        self.wspace = zeros((self.vswbins.shape[0],self.aspphi.shape[0],self.asptheta.shape[0],60,3,8,1,self.sec_det_dim))
    ...
    def _add_3Dv(self):
        ...
        if not "vxsw" in self.d.data.keys():
            self.d.add_data("vxsw",(self.d.data["vx"].T-self.d.data["vsw"]).T)
        else:
            self.d.data["vxsw"]=(self.d.data["vx"].T-self.d.data["vsw"]).T
    ...
    def _calc_pitchangle(self):
        ...
        # Cosmup stands for B-field is still important,i.e. inward field cosmup=1 means the opposite direction compared to the solar wind bulk than for outward polarity!
    ...
    def calc_w3dspecs(self,vswbins=arange(500.,800.1,10.),wxbins=arange(-2.,2.01,0.2),wybins=arange(-2.,2.01,0.2),wzbins=arange(-2.,2.01,0.2),min_whe=0.9,aspphi=(-20.,20.)):
        """
        Calculates w spectra. Data preselected by Master mask, i.e. Magnetic field direction.
        vsws -> bins for solar wind speed that are taken to calculate the instrumental coverage at w-bins
        """
        ...
        uvsw = self.d.get_data("Master","vsw")[Tallind]
        H,bs = histogramdd((uvsw,uasphi,uasptheta),bins=(vswbins,self.aspphi,self.asptheta))
        ivoffset = int(around(vswbins[0]-self.vswbins[0],-1)/10) # funktioniert so nur, wenn beide die gleiche Schrittgroesse haben
        for iv,v in enumerate(vswbins[:-1]):
            print iv,v
            for ip,p in enumerate(self.aspphi[:-1]):
                if (p>=aspphi[0])*(p<=aspphi[1]):
                    for it,t in enumerate(self.asptheta[:-1]):
                        if H[iv,ip,it]>0:
                            whe = self.vels/(v+5.)
                            epqs = arange(0,60,1)[whe>min_whe]
                            H2,bs = histogramdd((self.w3dspace[iv+ivoffset,ip,it,epqs,...,0,:].flatten(),self.w3dspace[iv+ivoffset,ip,it,epqs,...,1,:].flatten(),self.w3dspace[iv+ivoffset,ip,it,epqs,...,2,:].flatten()),bins=(wxbins,wybins,wzbins))
        ...
        for i in range(wxsw2.shape[1]):
            twts[:,i]=wgts*swgt
        H2,bs = histogramdd((wxsw2.flatten(),wysw2.flatten(),wzsw2.flatten()),bins=(wxbins,wybins,wzbins),weights=twts.flatten())
        self.d.remove_submask("He1+","wHe1+2")
        self.d.remove_submask("Master","vsw")
        self.d.remove_submask("Master","aspphi")
        self.d.remove_submask("Master","asptheta")
