      subroutine integralg(x1, y1, x2, y2, x3, y3, x4, y4, 
     & sigma, bb, result)
...
c    Die folgenden if Bedingungen dienen dazu, nicht weiter als im Winkelbereich 10*sigma zu integrieren.
       ...
       if((((gamma(alpha,1)) .gt. 50*sigma) .and. ((gamma(alpha,2))
     &   .gt. 50*sigma)) .or. (((-1*gamma(alpha,1)) .gt. 50*sigma)
     &   .and. ((-1*gamma(alpha,2) .gt. 50*sigma)))) then
       ...
       if((abs(gamma(alpha,1)) .gt. 50*sigma)
     &   .and. (abs(gamma(alpha,2)) .gt. 50*sigma)) then
  
            gamma(alpha,1)=signum(gamma(alpha,1))*50*sigma
            gamma(alpha,2)=signum(gamma(alpha,2))*50*sigma

       end if  
   
       if((abs(gamma(alpha,1)) .gt. 50*sigma)
     &   .and. (abs(gamma(alpha,2)) .le. 50*sigma)) then
  
            gamma(alpha,1)=signum(gamma(alpha,1))*50*sigma
       end if

  
       if((abs(gamma(alpha,2)) .gt. 50*sigma)
     &   .and. (abs(gamma(alpha,1)) .le. 50*sigma)) then
...
c     Integration über die relevante Fläche (Viereck)
      ...
      do while (x .le. y)
           ...
           result=result+abs((1/(2*pi*sigma*sigma*
     &     (1-0.326*sigma*sigma))*exp((-x*x)/(2*sigma*sigma)))
     &     *sin(x)*pi/180*sigma/100.)
           x=x+sigma/100.
