        subroutine geombeta(beta,sigma,defl,resprobst, resprobse)
     ...
     .      i,j,k
        ...
        do i =1,961
          do j =1,4
            betageom(i,j)=fitgeomall((i+961*nint(beta)),j)
c            write(*,*) "betageom =", betageom(i,j)
          end do
        end do
c        do i=1,961
c            write(*,*) "betageom =", (betageom(i,j),j=1,4)
c            write(*,*) "fitgeom =", (fitgeom(i,j),j=1,4)
c        end do
...
c        write(*,*) "defl =", defl
        i=0
        do while (nint(sigma*1000) .gt. nint(betageom(i+1,1)*1000))
            i=i+1
c            write(*,*) "i =", i, nint(betageom(i,1)*1000)
        end do
        j=0
        do while (nint(defl*1000) .gt. nint(betageom(j+1,2)*1000))
            j=j+1
c            write(*,*) "j =", j, nint(betageom(j+1,2)*1000)
        end do
c            write(*,*) "i =",i,"  j =", j
c            write(*,*) sigma, defl
        ...
        aux1=betageom(i+1+j-31,3)
        aux3=betageom(i+1+j-31,1)

        aux0=betageom(i+j-31,3)
        aux2=betageom(i+j-31,1)
...
c        write(*,*) "res0 =", res0
        ...
        aux1=betageom(i+j+1,3)
        aux3=betageom(i+j+1,1)
...
c        write(*,*) resprobst
        ...
        aux1=betageom(i+1+j-31,4)
        aux3=betageom(i+1+j-31,1)

        aux0=betageom(i+j-31,4)
        aux2=betageom(i+j-31,1)
