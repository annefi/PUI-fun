...
xdim=512
ydim=128
ion_names=["He2+","O6+","O7+","O8+","Ne8+","Mg10+","N5+","N7+","C5+","C6+","C4+","Si10+","S10+","Ca11+","Fe14+","Fe12+","Fe13+","Fe11+","Fe10+","Fe9+","Fe8+","Fe7+","O1+"]
#,"Fe7+","Fe8+","Fe9+","Fe10+","Fe11+"]

printall=0
listeions=[]
gp=Gnuplot()
#paraname1 = "/data/ivar/berger/testco/ion_para.dat"
paraname1 = "./new_ion_para.dat"
...
for ion in range(len(ion_names)):
    f=open(paraname1)
    #bis zum gew�nschten step springen
    ...
    for i in range(numpoints1):
        ...
        if (m[0]==ion_names[ion]):
           ...
           #x = sqrt(float(m[2])/(float(m[2])-0.42*charge[ion]))*float(m[1])+1.+(175-float(m[1]))*(step-25)*0.005
           #y = float(m[2])+1.-0.42*charge[ion]
...
#              x += 2.5
           ...
           points1.append([x+1.,y+1.,0.])
           xerrorp.append([x+1.,y+1.,1.,float(m[3]),0.,0.])
           xerrorm.append([x+1.,y+1.,1.,-float(m[3]),0.,0.])
           yerrorp.append([x+1.,y+1.,1.,0.,float(m[5]),0.])
           yerrorm.append([x+1.,y+1.,1.,0.,-float(m[5]),0.])
    if (not ionda):
...
data=open("/data/ivar/berger/binneddata/axlv2_sumwhol_1999_bin.dat")
