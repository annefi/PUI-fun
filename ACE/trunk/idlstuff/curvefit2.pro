...
;		PRINT,'curvefit2 : Param[',i,'] = ',a[i]
;		PRINT,'curvefit2 : FITA[',i,'] = ',fita[i]
;	ENDFOR
...
;	  testmult = (FLTARR(nparam)+1)*pder
;	  PRINT,'Curvefit2 : testmult = ',testmult
;   	  PRINT,'Curvefit2 : pder = ',pder
          beta = (nparam eq 1) ? [TOTAL((y-yfit)*Weights*pder[*,iparam])] : $
            (y-yfit)*Weights # pder[*,iparam]
;	  PRINT,'Curvefit : first beta = ',beta
;          PRINT,'Curvefit2 : TRANSPOSE(pder[*,iparam]) = ',TRANSPOSE(pder[*,iparam])	
          tmppder = fltarr(nY,nterms)
	  FOR i = 0,nterms-1 DO BEGIN
		  tmppder[*,i] = pder[*,i] * Weights[*]
	  ENDFOR		
	  alpha = TRANSPOSE(pder[*,iparam]) # tmppder[*,iparam]

;          alpha = TRANSPOSE(pder[*,iparam]) # $
;            (Weights # (FLTARR(nparam)+1)*pder[*,iparam])
;	  PRINT,'Curvefit2 : first alpha = ',alpha
;	  PRINT,'CUrvefit2 : alpha[diag] = ',alpha[diag]
