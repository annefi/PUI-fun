...
ll=0
sum=0.
system("rm hsf_results/linear/v_int_40/hsf_all_with_v")
for v_int in v2_int_array:
    ...
    for i in range(58):
        if (nepq > (abs(epq-epq_array[i]))):
            ...
            epqstep=i
    print epqstep
    file1=open("hsf_results/linear/v_int_40/hsf_all_with_v","a")
    for step in range(epqstep-8,epqstep+8):
    #for step in range(20,58):
        if(1==1):
            if (1==1):
                ...
                #gp("ktl3(x,y)=(1+((x-xkt3)**2/((ksl2)*(sktl3*(xkt3-"+str(dt)+"))**2)))**-(ksl2)")
                #gp("ktr3(x,y)=(1+((x-xkt3)**2/((exp(ksr1*(y-xge3))+ksr2)*(sktr3*(xkt3-"+str(dt)+"))**2)))**-(exp(ksr1*(y-xge3)+ksr2))")
            ...
            gp("set xrange[8:xge3+5]")
            #gp("set zrange[10:*]")
            for i in range(10):
                gp("set yrange[xkt3-3*sktl3*(xkt3-"+str(dt)+"):xkt3+3*sktr3*(xkt3-"+str(dt)+")]") # neuer fit, s.o.
                gp("fit s(x,y) '/data/wimmer/mu/jahresdaten/all_data/step_files/360_20_he/step_"+str(step)+"_o_cme' u 1:2:3:4 via Ge3")
                gp("fit s(x,y) '/data/wimmer/mu/jahresdaten/all_data/step_files/360_20_he/step_"+str(step)+"_o_cme' u 1:2:3:4 via Ge3,xkt3,xge3,sktr3,sktl3,sge3,ksr1,ksr2,ksl2")
            gp("set isosamples 50")
            #gp("set yrange[140:200]")
            gp("splot s(x,y), '/data/wimmer/mu/jahresdaten/all_data/step_files/360_20_he/step_"+str(step)+"_o_cme' u 1:2:3:4 with errorbars")
            ...
            for i in range(10):
                ...
                file1.write("   ")
            file1.write(str(steptov(step,4.,2.)))
            file1.write("\n")
    file1.close()
"""
file2=open("hsf_results/linear/all_under_he_speed/hsf_all_ave","w")
for i in range(58):
    file2.write(str(i))
    file2.write("   ")
