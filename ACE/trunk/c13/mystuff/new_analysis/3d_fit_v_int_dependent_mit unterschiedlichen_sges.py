...
ions=["He2","O8","N7","C6","13C6","O7","N6","C5","Mg10","Ne8","O6","N5","Si12"]
ions1=["He2","O8","N7","C6","13C6","O7","N6","C5","Mg10","Ne8","O6","N5","Si12"]
ions2=["He2","O8","N7","C6","13C6","O7","N6","C5","Mg10","Ne8","O6","N5","Si12"]
...
for v_int in v2_int_array:
    ...
    for step in range(epqstep-4,epqstep+4):
        ...
        for rn in range(1):
            if(1 == 1): 
                for i in range(len(ions)):
                    gp("gel"+str(i)+"(x)=((x-(xge"+str(i)+"))**2)**0.5<3*(sgel*((ev+xge"+str(i)+")-"+str(de)+"))? Ge"+str(i)+"*exp(-(x-(ev+xge"+str(i)+"))**2/(2*(sgel*((ev+xge"+str(i)+")-"+str(de)+"))**2)):0.")
                    gp("ger"+str(i)+"(x)=((x-(xge"+str(i)+"))**2)**0.5<3*(sger*((ev+xge"+str(i)+")-"+str(de)+"))? Ge"+str(i)+"*exp(-(x-(ev+xge"+str(i)+"))**2/(2*(sger*((ev+xge"+str(i)+")-"+str(de)+"))**2)):0.")
                    ...
                    # zusammensetzen der gauss funktion
                    gp("ge"+str(i)+"(x)=x<xge"+str(i)+"? gel"+str(i)+"(x):ger"+str(i)+"(x)")
                    ...
                    gp("sgel="+str(he_data_2[teststep,3]))#*random.gauss(1.,0.1)))
                    gp("sger="+str(he_data_2[teststep,3]))#*random.gauss(1.,0.1)))
                    gp("ev=0.")
                    gp("tv=0.")
                ...
                for i in range(3):
                    ...
                    gp("fit s(x,y) '/data/wimmer/mu/jahresdaten/all_data/step_files/"+str(v_int)+"_"+str(interval)+"_he/step_"+str(step)+"_o_cme' u 1:2:3:4 via Ge1,Ge2,Ge3")
                    gp("set yrange["+str(int(tp[3,0]-8))+":"+str(tp[3,0]+45)+"]")
