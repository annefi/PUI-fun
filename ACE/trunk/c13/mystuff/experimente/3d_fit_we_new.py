...
ions=["He2","O8","N7","C6","Mg11","13C6","O7","N6","Mg10","C5","Ne8","Mg9","O6","N5","C4"]
...
#system("rm res_16")
v_int_array=range(345,345,10)
interval=int(20)
v2_int_array=range(340+interval,380,interval*2)
c12=zeros(len(v2_int_array),Float)
c13=zeros(len(v2_int_array),Float)
c13c12=zeros(len(v2_int_array),Float)
dummy1=zeros([10,1000,len(v2_int_array)],Float)
dummy2=zeros([10,1000,len(v2_int_array)],Float)
epq=0.
ll=0
        ...
        for rn in range(1000):
            if(1 == 1): 
                for i in range(len(ions)):
                    ...
                    gp("kappatl="+str(he_data[int(v_int),step,11]*random.gauss(1.,0.05)))
                    ...
                    #gp("sge="+str(he_data_2[step,3]))#*random.gauss(1.,0.1)))
                ...
                for i in range(3):
                    ...
                    #gp("fit s(x,y) '/data/kleopatra/mu/jahresdaten/all_data/step_files/"+str(v_int)+"_he/step_"+str(step)+"_we' u 1:2:3:4 via Ge1,Ge2,Ge3")
                    #gp("set yrange["+str(int(tp[5,0])+2)+":200]")
                    #gp("fit s(x,y) '/data/kleopatra/mu/jahresdaten/all_data/step_files/"+str(v_int)+"_he/step_"+str(step)+"_we' u 1:2:3:4 via Ge6,Ge7,Ge8,Ge9,Ge10,Ge11,Ge12,Ge13")
                    #gp("set yrange[150:200]")
                    #gp("fit s(x,y) '/data/kleopatra/mu/jahresdaten/all_data/step_files/"+str(v_int)+"_he/step_"+str(step)+"_we' u 1:2:3:4 via Ge1,Ge2,Ge3,Ge5,Ge6,Ge7,Ge8,Ge9,Ge10,Ge11,Ge12,Ge13")
                    gp("set xrange["+str(tp[3,1]-5)+":"+str(tp[3,1]+20)+"]")
                    gp("set yrange["+str(int(tp[3,0]-10))+":"+str(int(tp[3,0]+4))+"]")
                    gp("fit s(x,y) '/data/wimmer/mu/jahresdaten/all_data/step_files/"+str(v_int)+"_"+str(interval)+"_he/step_"+str(step)+"_we' u 1:2:3:4 via Ge1,Ge2,Ge3")
                    gp("set yrange["+str(int(tp[5,0])+2)+":"+str(tp[3,0]+35)+"]")
                    gp("fit s(x,y) '/data/wimmer/mu/jahresdaten/all_data/step_files/"+str(v_int)+"_"+str(interval)+"_he/step_"+str(step)+"_we' u 1:2:3:4 via Ge6,Ge7,Ge8,Ge9,Ge10,Ge11,Ge12,Ge13")
                    gp("set yrange["+str(int(tp[3,0]-10))+":"+str(tp[3,0]+35)+"]")
                    gp("fit s(x,y) '/data/wimmer/mu/jahresdaten/all_data/step_files/"+str(v_int)+"_"+str(interval)+"_he/step_"+str(step)+"_we' u 1:2:3:4 via Ge1,Ge2,Ge3,Ge5,Ge6,Ge7,Ge8,Ge9,Ge10,Ge11,Ge12,Ge13")
                ...
                file0=open(str(v_int)+"_"+str(step)+"_new_opf","a")
