...
system("rm neue_fitergebnisse/ungebinnt/fit_res_all_lk_rk_obg" )
for ion in ["o2","fe","si","h","mg","ne","o3","s"]:
    ...
    for n in swxli:
        ...
        try:
            file1=open("/home/kleopatra/mu/daten/giessen/swxli"+str(n)+".hste")
        except IOError:
            print "swxli"+str(n)+".hste existiert nicht"
            continue
        p=0
        for i in file1:
            x=i.split()
...
# ToF
        ...
        rlp = 0.
        twarning = int(0)
        for i in range(int(hp),int(hp)-50,-1):
            if (rld > hd*0.01):
                rld = tof_data[i,3]
                rlp = float(i)
        if (int(rlp) == hp-49):
        ...
        if (twarning == 1):
            print "**** ToF - WARNUNG **** LINKE RANGE IST HP -49"
        if (twarning == 2):
            print "**** ToF - WARNUNG **** RECHTE RANGE IST 1021"
        if (twarning == 3):
...
# Energie
        ...
        if (ewarning == 1):
            print "**** E - WARNUNG **** LINKE RANGE ISt 10"
        if (ewarning == 2):
            print "**** E - WARNUNG **** RECHTE RANGE IST 255"
        if (ewarning == 3):
...
# ToF
        ...
        gp("fit s(x) '~/daten/giessen/swxli"+str(n)+".hstt' u 1:2:3 via G1,sk1,sk2,x1,kappa1,kappa2")
        gp("plot  s(x), '/home/kleopatra/mu/daten/giessen/swxli"+str(n)+".hstt' u 1:2:3 with errorbars")
...
# Energie
        ...
        gp3("kappa1=2.")
        gp3("kappa2=2.")
        gp3("s(x)=x<x1 ? k1(x):k2(x)")
        gp3("fit s(x) '~/daten/giessen/swxli"+str(n)+".hste' u 1:2:3 via G1,sk1,sk2,x1,kappa1,kappa2")
        gp3("set xrange[1:"+str(int(errp)+50)+"]")
        gp3("plot  s(x), '/home/kleopatra/mu/daten/giessen/swxli"+str(n)+".hste' u 1:2:3 with errorbars")
...
#       1. Fitvorgang abgeschlossen
    ...
    if (do == 1):
        ...
        file2=open("neue_fitergebnisse/ungebinnt/fit_res_"+ion+"_lk_rk_obg","w")
        for i in swxli:
            if (int(ehpl1[i]) >= 25):
                if (int(res[i,4]) >= 500):
                    file1.write(str(i))
