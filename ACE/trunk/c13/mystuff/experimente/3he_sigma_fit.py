...
tp=zeros([58,2],Float)
res=zeros([10],Float)
ksr1=zeros([300],Float)
ksr2=zeros([300],Float)
...
ksl1=zeros([300],Float)
ksl2=zeros([300],Float)
...
sktl3=zeros([300],Float)
sktr3=zeros([300],Float)
...
for v_int in v_int_array:
    ...
    for i in range(58):
        if (nepq > (abs(epq-epq_array[i]))):
            ...
            epqstep=i
    print epqstep
    file1=open("res_he_3","a")
    for step in range(epqstep-2,epqstep+3):
        if(1==1):
            if (1==1):
                # Energie + ToF Gauss - alle gleiches sge und sgt, nämlich sge3 und sgt3, sge(i)=xge(i)*sge3
                gp("ge3(x)=Ge3*exp(-(x-xge3)**2/(2*(sge3*(xge3-0.682817))**2))")

                # tof kappa funktion mit variablem kappa für jeden energiekanal
                #gp("ktl3(x,y)=(1+((x-xkt3)**2/((kappatl3)*(sktl3*(xkt3-5.80315))**2)))**-(kappatl3)")
                gp("ktl3(x,y)=(1+((x-xkt3)**2/((max1((ksl1*(y-xge3))+ksl2))*(sktl3*(xkt3-5.80315))**2)))**-(max1((ksl1*(y-xge3))+ksl2))")
                gp("ktr3(x,y)=(1+((x-xkt3)**2/((max1((ksr1*(y-xge3))+ksr2))*(sktr3*(xkt3-5.80315))**2)))**-(max1((ksr1*(y-xge3))+ksr2))")
                ...
                gp("sge3="+str(0.11))#*random.gauss(1.,0.1)))

            gp("s(x,y)=s3(x,y)")#+s2(x,y)+s3(x,y)+s4(x,y)+s5(x,y)+s6(x,y)+s7(x,y)+s8(x,y)+s9(x,y)+s10(x,y)+s11(x,y)+s12(x,y)+s13(x,y)")
            gp("set xrange[8:13]")
            for i in range(10):
                gp("set yrange["+str(tp[step,0]-5)+":"+str(tp[step,0]+5)+"]")
                gp("fit s(x,y) '/data/kleopatra/mu/jahresdaten/all_data/step_files/"+str(v_int)+"/step_"+str(step)+"_we' u 1:2:3:4 via Ge3")
                gp("fit s(x,y) '/data/kleopatra/mu/jahresdaten/all_data/step_files/"+str(v_int)+"/step_"+str(step)+"_we' u 1:2:3:4 via Ge3,xkt3,xge3,sktr3,sktl3")

            gp("set isosamples 50")
            gp("set yrange[130:160]")
