...
int main(int argc, char* argv[]){
  ...
  /* ----------------------------------------------------------------------- */
  if (DoProbrates) { // output fmcomp block
  ...
  char hdrfmt[] ="# %6s %6s %8s %6s %6s %6s %6s %6s %6s %6s %6s %7s\n";
  char datafmt[]=
  "  %6.2f %6s %8.2e %6.2f %6.2f %6.2f %6.2f %6.2f %6.2f %6.2f %6.2f %3s %3s\n";

  fprintf(FMCF,hdrfmt,"E/q","ion","Prate","tof","tofsig","pctof","deltof",
	  "esd","esdsig","pcesd","delesd","del>sig");
  ...
  //for (int nedb = 41; nedb < 42; nedb++){ // for debugging
	...
	esdout = abs(delesd - sesd);
      ...
      fprintf(FMCF, datafmt, 
	      ad.Eqtab[nedb],ad.Ions[s].name.c_str(),PR.getProbRate(nedb,s),
	      tof,stof,pctof,deltof,
	      esd,sesd,pcesd, delesd,
  ...
  /* ----------------------------------------------------------------------- */
  /* ---------------------- Output Primary Ion Table ----------------------- */
  /* ----------------------------------------------------------------------- */
  string outfile = filebase + "_ions.dat";
  FILE *OUT;
  OUT = fopen(outfile.c_str(), "w+");

  // keep fields same total width!
  char hdrfmt[] = "%13s %9s %9s %9s %9s %9s %9s %9s %9s\n";
  char datafmt[] = "%13s %9.2e %9.2e %9.2e %9.2e %9.2e %9.2e %9.2e %9.2e\n";

  fprintf(OUT, hdrfmt,"Species","BoxRate","SpillRate","ProbRate","Density",
	  "v","vth","TofFcd","EsdFcd");

  float ratesum;  // PR sum over charge steps for species s
  float weight;  // prob rate at edb for s / ratesum

  // fractional center differences (fcd)
  // --- for current edb
  float tofFcd, esdFcd;
  // --- averaged over edbs
  float avTofFcd, avEsdFcd;

  TNT::Matrix<float> pcmat;  // centers for all s at this nedb

  // temporary loop variables

  for (int s = 0; s < ad.Ions.size(); s++){ // loop over species

    // calc. sum of all rates for normalization
    ratesum = 0.0;
    for (int nedb = FIRSTEDB; nedb < MAXEDB; nedb++){
      ratesum += PR.getProbRate(nedb,s);
    }

    // calc. average fractional center differences (fcd)
    avTofFcd = 0.0;
    avEsdFcd = 0.0;
    for (int nedb = FIRSTEDB; nedb < MAXEDB; nedb++){
      pcmat = PR.getProbCenter(nedb);
      tofFcd = ( pcmat[s][0] - ad.Ions[s].tof[nedb]) / ad.Ions[s].tof[nedb];
      esdFcd = ( pcmat[s][1] - ad.Ions[s].esd[nedb]) / ad.Ions[s].esd[nedb];

      weight = PR.getProbRate(nedb,s)/ratesum;
      if (weight >= 0.1){ // arb. cutoff
	  avTofFcd += tofFcd * weight;
	  avEsdFcd += esdFcd * weight;
      }
	
    }

    // finish average
    avTofFcd = avTofFcd/(MAXEDB - FIRSTEDB);
    avEsdFcd = avEsdFcd/(MAXEDB - FIRSTEDB);

    fprintf(OUT, datafmt, ad.Ions[s].name.c_str(), 
	    BR.getBoxrate(s),SR.getCorSpillRate(s),PR.getProbRate(s),
	    DF.getDensity(s),DF.getBulkVelocity(s),DF.getThermVelocity(s),
	    avTofFcd,avEsdFcd);
  }
  ...
  /* ----------------------------------------------------------------------- */
  if (1) { // output comparison table
    string outfile = filebase + "_boxcomp.dat";
    FILE *OUT;
    OUT = fopen(outfile.c_str(), "w+");
    ...
    cout << endl;
    cout << thisprog << " -I- writing comparison table..." << endl;

    fprintf(OUT,"\n");
    fprintf(OUT,"%6s\t%8s\t%8s\t%8s\t%8s\n", "", "Box", "Spill", 
	   "Prob", "Density");
    fprintf(OUT,
      "----------------------------------------------------------------\n");
    ...
    for (int i = 0; i < ctions.size(); i++){
      ...
      fprintf(OUT,fmt, ad.Ions[s].name.c_str(), BR.getBoxrate(s),
    ...
    fprintf(OUT,
      "----------------------------------------------------------------\n");
    fprintf(OUT,fmt, "^sum^", boxsum, spillsum, probsum, denssum);

    fprintf(OUT,
      "----------------------------------------------------------------\n");
    fprintf(OUT,fmt, "O7/O6", o7o6br, o7o6sr, o7o6pr, o7o6df);
    ...
    fprintf(OUT,fmt, "C/O", csums[0]/osums[0], csums[1]/osums[1],
