...
class ProbRates {
 public:
  ...
  vector<float> calcProbCenter(const vector<Ion>& grp, 
			       const MeasurementArray& MA,
			       int s, int nedb);
  // ----- for species s, all charge steps
  vector<vector<float> > calcProbCenter(const vector<Ion>& grp,
					const MeasurementArray& MA,
					int s);
 ...
 private:
  ...
  // prob rates, MA Counts Distributed by probability in P (CD)
  vector<TNT::Vector<float> > Nprob;   // CD -- Counts Distributed

  // prob rates, Counts to species with Max probability
  vector<TNT::Vector<float> > NprobCM; // CM -- Counts to Max
