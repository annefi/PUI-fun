...
;; define subsections
xrs = [[300,425],[400,700],[0,1023]]
yrs = [[20,100],[20,80],[0,255]] 

;; ----------------------------------------------------------------------------
;;                           get to work
;; ----------------------------------------------------------------------------
;for nstep=2,59 do begin ; loop over charge steps
for step=41,41 do begin ; loop over charge steps

  nstep = step - 2      ; loops start at zero even though element zero 
                        ; represents EDB 2 

  print, 'fmcomp -I- working on step ' + string(format='(I)',nstep)
  ...
  for isub=0,1 do begin
    ...
    ;; plot ET matrix
    subtitle = psfile + ', step = ' + string(format='(I)',nstep)
    color_plot_jmr, submat,  xrange, yrange, xtitle=xtitle,subtitle=subtitle,$
      ytitle=ytitle,  $  
      /fill, title=title ,  /logsc, zrange=zrange, zmin_color=white
...
;
pro fix_ct_ends, reverse=reverse
