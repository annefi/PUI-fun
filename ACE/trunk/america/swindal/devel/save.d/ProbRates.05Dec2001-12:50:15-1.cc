...
int ProbRates::calc(const vector<Ion>& grp, const MeasurementArray& MA, 
  ...
  for (int s = 0; s < grp.size(); s++){
    G.push_back(zero);
    P.push_back(zero);
    //names[s] = grp[s].name;  // store names
...
vector<float> ProbRates::calcProbCenter(const vector<Ion>& grp, int s){
  /*  Description:
        ...
        Checks to be sure element s of grp is the same as element s
        from getProbRate().
...
vector<vector<float> > ProbRates::calcProbCenter(const vector<Ion>& grp){
  /*  Description:
        ...
        Calculates the probabilistic center of the peak for all
        species in grp, using calcProbCenter for specified s.
 
  */
  vector<vector<float> > centers;
  
  for (int s = 0; s < grp.size(); s++){
    centers.push_back(calcProbCenter(grp, s));
  }
