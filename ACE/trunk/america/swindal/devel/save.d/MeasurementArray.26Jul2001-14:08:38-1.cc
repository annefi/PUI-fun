...
TNT::Matrix<float> MeasurementArray::EtSlice2Mmq(TNT::Matrix<float>, int nedb){
  ...
  const int NM = 122;  // Number of mass bins
  const int NQ = 124;  // Number of m/q bins

  TNT::Matrix<float> mmq(NM, NQ, 0.0);
  ...
  for (int ntof = 0; ntof < MAXTOFCH; ntof++){
    for (int nesd = 0; nesd < MAXESDCH; nesd++){
      ...
      /* Note: All the special mmch channels, ie. > 122 (NM), cannot
  ...
  for (int ntof = 0; ntof < MAXTOFCH; ntof++){
    for (int nesd = 0; nesd < MAXESDCH; nesd++){
      ...
      im = (NM - 1) * int ( log(mm/mm0)/log(mm1/mm0) );

      iq = (NQ - 1) * int ( log(mq/mq0)/log(mq1/mq0) );

      if ( (im >= 0 && iq >= 0) &&  (im <= NM && iq <= NQ) ){
