...
AnalysisData::AnalysisData(void){
  ...
  /* -------------------------------------------------------
     ...
     -------------------------------------------------------
  */
  EqAcceptFac = 0.043;   // from swindal.h ref. 1
...
//    cout << endl;
  ...
  */
  ...
  string name;
  for (int nion = 0; nion < Ions.size(); nion++){
    Ions[nion].tof.reserve(MAXEDB);
    Ions[nion].stof.reserve(MAXEDB);
    Ions[nion].esd.reserve(MAXEDB);
    Ions[nion].sesd.reserve(MAXEDB);
