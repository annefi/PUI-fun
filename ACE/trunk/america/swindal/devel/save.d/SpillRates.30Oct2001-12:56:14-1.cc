...
int SpillRates::calc(vector<Ion>& grp, BoxRates BR){
  ...
  for (int nedb = FIRSTEDB; nedb < MAXEDB; nedb++){
    ...
    if (result != 0){ // some species are misbehaving, remove them for
      ...
      pos = unique(badspecies.begin(),badspecies.end()); 
      badspecies.erase(pos, badspecies.end()); // resizes too

      // remove misbehaving ions from group
      int newsize = grp.size() - badspecies.size();
      vector<Ion> newgrp;
	...
	// check through badspecies for s
	...
	// copy s to new group if it was not removed
	if (! Removed[nedb][s]){
	  if (t < newsize){
	    //newgrp[t]= grp[s];
      ...
      // re-run calc2
      // pass a 1D indexed sub-vector into calc2 to replace resize
      // should work anyway
//       NspillC2.resize(grp.size());
//       NspilltilC2.resize(grp.size());
      result = calc2(newgrp,nedb,newB,NspillC2,NspilltilC2,badspecies);
