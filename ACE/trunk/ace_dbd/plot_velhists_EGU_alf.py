	def plot_ionvelhists(velrange_ion,velrange_vsw,velbinsize,ionname,element,charge,vion_sync,vsw_sync,robustmin=1,norm=False,plot_aellig=True,legendsize=16):
        ...
        for h in h1:
            if sum(h):
                means.append(average(velrange_ion[:-1],weights=h))
            else:
                means.append(0.)
		...
		axarr[0].plot(velrange_vsw[:-1]+0.5*velbinsize[1],means,linewidth=2.5,color="b",label="robust mean ion velocity")#plot robust 
		...
		if plot_aellig==True:
			...
			if ionname=="O6+":
				axarr[0].plot(Id,Hefti_O6(Id),color="k",linestyle="--",linewidth=2.5, label="mean ion velocity obtained\nby Hefti (1998)")	
			if ionname=="Si7+":
		...
		#axarr[0].legend(loc="lower left",prop={'size':20})
		axarr[0].axis([velrange_vsw[0],velrange_vsw[-1],velrange_ion[0],velrange_ion[-1]])
		...
		#axarr[0].text(300,600,r"$\mathrm{%s}^{%i+}$"%(element,charge),bbox={'facecolor':'white','alpha':1.0,'pad':8}, fontsize=45)
		#axarr[0].annotate(r"$\mathrm{%s}^{%i+}$"%(element,charge), xy=(0.025, 0.80), xycoords='axes fraction',bbox={'facecolor':'white','alpha':1.0,'pad':18}, fontsize=54)
		axarr[0].annotate(r"$\mathrm{%s}$"%(element), xy=(0.025, 0.80), xycoords='axes fraction',bbox={'facecolor':'white','alpha':1.0,'pad':18}, fontsize=54)
		#axarr[0].text(0.45,0.1,"Total number of cases: %i"%(N_cases/58),
#bbox={'facecolor':'white','alpha':1.0,'pad':8}, fontsize=20)		
		#axarr[0].text(280,600,r"$\mathrm{Fe}^{10+}$", fontsize=45)
		axarr[0].plot([0,0],[1,1],color="white",label="Total number of cases: %i"%(N_cases/58))
		axarr[0].legend(loc="lower right",prop={'size':legendsize})
		plt.show()
        ...
        axarr[1].bar(velrange_vsw[:-1],means,width=velbinsize[1],color="b")#plot robust 
		#axarr[1].bar(velrange_vsw[col_yrobustmeans>0],veldiff,width=velbinsize,color="m")
		
		if plot_aellig==True:		
			axarr[1].bar(velrange_vsw[col_yrobustmeans>0],Hefti_Fe9(velrange_vsw[col_yrobustmeans>0])-velrange_vsw[col_yrobustmeans>0],width=velbinsize[1],color="k",alpha=0.5)
		...
		axarr[1].axis([velrange_vsw[0],velrange_vsw[-1],-0.5,1])
		...
		#axarr[1].errorbar(velrange_vsw[col_yrobustmeans>0]+0.5*velbinsize[1],col_yrobustmeans[col_yrobustmeans>0]-velrange_vsw[col_yrobustmeans>0],yerr=col_yUs,linestyle="None",marker="_",markersize=10,color="k")
		plt.show()
		...
		if norm==True:
			cb.set_label(r'$\mathrm{Rel.\ frequency\ of\ occurrence\ per\ bin}$', rotation=90, fontsize=25)
		else:
			cb.set_label(r'$\mathrm{Number\ of\ occurrences\ per\ bin}$', rotation=90, fontsize=25)
		plt.show()
