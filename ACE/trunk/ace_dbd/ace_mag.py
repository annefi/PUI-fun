...
class ace_mag(dbData):
    ...
    def bin_data(self,time1,time2,prod1="d00",prod2="time"):
        """
        ...
        """
        mYear,x=histogram(self.get_data("Master",prod1),time1,weights=self.get_data("Master","year"))
        nrT,x=histogram(self.get_data("Master",prod1),time1)
        self.data["year"]=mYear/nrT
        ...
        indT=searchsorted(time1,self.get_data("Master",prod1))-1
        indT=indT[(indT>=0)*indT<time1.shape[0]-1]
        nrT,x=histogram(self.get_data("Master",prod1),time1)
        mBx,x=histogram(self.get_data("Master",prod1),time1,weights=self.get_data("Master","Bx"))
        mBy,x=histogram(self.get_data("Master",prod1),time1,weights=self.get_data("Master","By"))
        mBz,x=histogram(self.get_data("Master",prod1),time1,weights=self.get_data("Master","Bz"))
        mB=sqrt((mBx/nrT)**2+(mBy/nrT)**2+(mBz/nrT)**2)
        malpha,x=histogram(self.get_data("Master",prod1),time1,weights=self.get_data("Master","alpha"))
        sB,x=histogram(self.get_data("Master",prod1),time1,weights=(self.get_data("Master","B")-mB[indT])**2)
        sBx,x=histogram(self.get_data("Master",prod1),time1,weights=(self.get_data("Master","Bx")-(mBx/nrT)[indT])**2)
        sBy,x=histogram(self.get_data("Master",prod1),time1,weights=(self.get_data("Master","By")-(mBy/nrT)[indT])**2)
        sBz,x=histogram(self.get_data("Master",prod1),time1,weights=(self.get_data("Master","Bz")-(mBz/nrT)[indT])**2)
        salpha,x=histogram(self.get_data("Master",prod1),time1,weights=(self.get_data("Master","alpha")-(malpha/nrT)[indT])**2)
        sBxm,x=histogram(self.get_data("Master",prod1),time1,weights=(self.get_data("Master","dBxm")**2))
        sBym,x=histogram(self.get_data("Master",prod1),time1,weights=(self.get_data("Master","Bym")**2))
        sBzm,x=histogram(self.get_data("Master",prod1),time1,weights=(self.get_data("Master","Bzm")**2))
        
        self.data[prod1]=time1[:-1]
        self.data[prod2]=time2
        self.data["B"]=mB
        self.data["Bx"]=mBx/nrT
        self.data["By"]=mBy/nrT
        self.data["Bz"]=mBz/nrT
        self.calc_angles()
        if self.data.has_key("sB"):
            self.data["sB"]=sqrt(sB/nrT)
        else:
            self.add_data("sB",sqrt(sB/nrT))
        if self.data.has_key("sBx"):
            self.data["sBx"]=sqrt(sBx/nrT)
        else:
            self.add_data("sBx",sqrt(sBx/nrT))
        if self.data.has_key("sBy"):
            self.data["sBy"]=sqrt(sBy/nrT)
        else:
            self.add_data("sBy",sqrt(sBy/nrT))
        if self.data.has_key("sBz"):
            self.data["sBz"]=sqrt(sBz/nrT)
        else:
            self.add_data("sBz",sqrt(sBz/nrT))
        if self.data.has_key("salpha"):
            self.data["salpha"]=sqrt(salpha/nrT)
        else:
            self.add_data("salpha",sqrt(salpha/nrT))
        if self.data.has_key("sBxm"):
            self.data["sBxm"]=sqrt(sBxm/nrT)
        else:
            self.add_data("sBxm",sqrt(sBxm/nrT))
        if self.data.has_key("sBym"):
            self.data["sBym"]=sqrt(sBym/nrT)
        else:
            self.add_data("sBym",sqrt(sBym/nrT))
        if self.data.has_key("sBzm"):
            self.data["sBzm"]=sqrt(sBzm/nrT)
        else:
            self.add_data("sBzm",sqrt(sBzm/nrT))
