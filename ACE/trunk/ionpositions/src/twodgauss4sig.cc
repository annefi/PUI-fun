...
double twodgauss4sig(int x,int y, vdouble &param, vdouble &deriv, int numpar, int numion, vint &iparam, double range)
{
  ...
  for (int i = 0; i < numion;i++){
    ...
    //    if ((dx <= range * param[sigxp]) && (dx >= -(range * param[sigxm])) && (dy <= range * param[sigyp]) && (dy >= -(range * param[sigym])) && (param[sigxp]> 0.) && (param[sigxm]>0.) && (param[sigyp]> 0.) && (param[sigym]>0.) && (param[A]>0.) && (param[x0]>0.) && (param[y0]>0.)){
