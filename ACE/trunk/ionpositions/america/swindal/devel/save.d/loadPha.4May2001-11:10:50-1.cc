...
int loadPha(vector<Pha> *pha){
  ...
  vector<Pha> cycpha;    // PHA words from current cycle

  /* -- stats variables -- */
  // for proton speed
  double epq[MAXEDB];    // set of epq valus used for weights
  int nphacyc[MAXEDB];   // number of pha words per EDB
  ...
  double weight[MAXEDB]; // factor to scale nphacyc to actual if all
  ...
  while (a3rcresult == 0 && ncycle <= ncyclemax){
    ...
    */
	...
	nphacyc[nedb] = 0;
      ...
      for (npha = 0; npha < nphacyc[nedb]; npha++){
    ...
    hspeed = swspeed(epq,nphacyc,1.0,0.8); // moq = 1.0, cutoff = 0.8
    ...
    for (i = 0; i < cycpha.size(); i++){
      cycpha[i].weight = weight[cycpha[i].nedb];

      cycpha[i].hspeed = hspeed;
      
      // add 
      pha->push_back(cycpha[i]);
      // remove from cycpha instead of initializing?
    }
