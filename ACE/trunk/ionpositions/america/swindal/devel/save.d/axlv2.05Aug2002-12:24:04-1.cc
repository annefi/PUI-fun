...
int main(int argc, char* argv[]){
  ...
  if (LoadMA) {         // read MeasurementArray from file
	...
	cout << thisprog << " -I- using file " << gCurLv1File << endl;
      ...
      // this object contains ion info, Eq table, foward model, PAV
      // reads some info from gCurLv1File
      ad.init();
      if (DbgLvl >= 2) ad.dump();
  ...
  for (int nedb = FIRSTEDB; nedb < MAXEDB; nedb++){
    ...
    for (int s = 0; s < ad.Ions.size(); s++){ // loop over species
      ...
      /* calculate center difference as prob center - fm center */
      tofDif = pcmat[s][0] - ad.Ions[s].tof[nedb];
      esdDif = pcmat[s][1] - ad.Ions[s].esd[nedb];
