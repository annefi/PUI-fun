...
*/
  ...
  for (pha_it = pha->begin(); pha_it != pha->end(); pha_it++){
	...
	cout << thisprog << "-D- after add MAcounts[" << pha_it->nedb << "," 
	     << pha_it->tofch << "," 
	     << pha_it->esdch << "]=" 
	     << MAcounts[pha_it->nedb][pha_it->tofch][pha_it->esdch] << endl << endl;
      }

    }
  }

  cout << thisprog << " -D- nphatot=" << nphatot << endl;;
  ...
  if (timeInterval > 0) {
	...
	for (int nesd = 0; nesd < MAXESDCH; nesd++){
	  ...
	  if (DbgLvl > 3 && MA[nedb][ntof][nesd] > 0.0){
	  ...
	  MA[nedb][ntof][nesd] += MAcounts[nedb][ntof][nesd];
