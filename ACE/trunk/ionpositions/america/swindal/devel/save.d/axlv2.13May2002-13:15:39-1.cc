...
int main(int argc, char* argv[]){
  ...
  //string step;
  for (int nedb = FIRSTEDB; nedb < MAXEDB; nedb++){
    sprintf(step, "%02d", nedb);
    //step = stepp;
    outfile = filebase + "_ions_" + step + ".dat";
    ...
    for (int s = 0; s < ad.Ions.size(); s++){ // loop over species
      ...
      /* calculate fractional center difference (fcd) as a measure of distance
	 between fm center and prob center. */
      ...
      fprintf(OUT, datafmt, ad.Ions[s].name.c_str(), 
	      BR.getBoxrate(nedb,s),SR.getCorSpillRate(nedb,s),
