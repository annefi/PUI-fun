...
;                           size).  J. Raines
;               31Jul2003 - Changed all instances of !d.n_colors 
;                           to !d.table_size
;
; Revision controlled by CVS:
; $Id: color_plot.pro,v 1.2 2003/07/31 15:01:42 jraines Exp $
;==========================================================================
...
IF (!D.NAME EQ 'PS') THEN BEGIN
  ...
  IF ((N NE S(1)) OR (M NE S(2))) AND ((N+1 NE S(1)) OR (M+1 NE S(2))) $
    ...
    IF NOT KEYWORD_SET(ZCOLORS) THEN $
      DATA = round((!d.table_size-1) * (DATA-DMIN) / (DMAX - DMIN))

    FDATA = (!d.table_size-1) * (DATA-DMIN) / (DMAX - DMIN)
...
IF NOT KEYWORD_SET(ZCOLORS) THEN $
  DATA = round((!d.table_size-1) * (ZBIG-DMIN) / (DMAX - DMIN)+.01) $
ELSE DATA=ZBIG
FDATA = (!d.table_size-1) * (ZBIG-DMIN) / (DMAX - DMIN)+.01
...
IF KEYWORD_SET(CONTOUR) THEN BEGIN
  ...
  DATA = FDATA * (DMAX-DMIN) / (!d.table_size-1)  + DMIN
