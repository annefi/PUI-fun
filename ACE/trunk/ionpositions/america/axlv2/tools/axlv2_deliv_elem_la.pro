; Filename: axlv2_deliv_elem_la.pro
;
; Description:
;   Plot data from axlv2_deliv_elem_la*.dat files.
;
...
; I/O Arg         Explanation
; --- ----------  -----------
; I   file        axlv2_deliv_elem_la*.dat data file
; O   data        axlv2_deliv_elem_la data structure
...
pro axlv2_deliv_elem_la, file=file, data=d, tag=tag,$
                     doyfr_ranges=doyfr_ranges

thisprog = 'axlv2_deliv_elem'

;; --- extract elements from filename ---
;; proto. filename: axlv2_deliv_elem_la_2003326_2003350.dat
file_parse, file=file, base=base, split_base=sb
ydoy_str = sb[4]
;; --- grab year string
tmp = split_ydoy(sb[4], /strings)
year_str = tmp[0]
...
if (n_elements(sb) eq 6) then begin
  tmp = split_ydoy(sb[5])
  doy2 = tmp[1]
...
;; defaults
;  S/O, N/O, 3He/4He (plot order: bottom to top)
yranges = [[1.e-2,1.e0],[1.e-1,1.e1],[1.e-4,1.e0]]
...
;; ----------------------------------------------------------------------
;;read swics data
restore, file='axlv2_deliv_elem_la_template.save'
...
for i_range = 0, num_doyfr_ranges-1 do begin
  ...
  switch_x_labels, /off
  nyrange = 3
  plot, d.doyfr, d.i3he_4he, ytitle= '!E3!NHe/!E4!NHe', min=0., xrange=range,  $
        position=positions[nplot--,*], ylog=YLOG, yrange=yranges[*,--nyrange],$
        yticks=4

  plot, d.doyfr, d.n_O, ytitle= 'N/O', min=0., xrange=range,  $
        position=positions[nplot--,*], ylog=YLOG, yrange=yranges[*,--nyrange],$
        yticks=2

  switch_x_labels, /on
  plot, d.doyfr, d.S_O, ytitle= 'S/O', min=0., xrange=range, $
        position=positions[nplot--,*], ylog=YLOG, yrange=yranges[*,--nyrange],$
        yticks=2
