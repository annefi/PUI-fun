...
char *argv[];
{
  ...
  if( init_rd_swepam_dloadi(hdf_fp, SD_id, "r") <= 0) {
    fprintf(stderr,"No swepam dloadi data\n");
    exit(-1);
  }

  /* read and print data */
  ii=0;
  while((retval=read_swepam_dloadi(&testdata,ii))!=-1) {
    printf("ii=%4d %10u %3d \n", 
	    ii++, testdata.output_sctime, testdata.QAC);
    for(jj=0;jj<16;jj++) {
    printf("\t%4d %4d %4d %4d %4d %4d %4d\n", 
	    testdata.MinFrmIndx[jj], testdata.EventFlag[jj], 
	    testdata.StatRec[jj][0], testdata.LoadErrRec[jj][0], 
