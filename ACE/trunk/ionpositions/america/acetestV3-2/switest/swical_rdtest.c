...
char *argv[];
{
  ...
  if( init_rd_swepam_dcali(hdf_fp, SD_id, "r") <= 0) {
    fprintf(stderr,"No swepam dcali data\n");
    exit(-1);
  }

  /* read and print data */
  ii=0;
  while((retval=read_swepam_dcali(&testdata,ii))!=-1) {
    printf("\nii=%4d %10u %3d \n", 
