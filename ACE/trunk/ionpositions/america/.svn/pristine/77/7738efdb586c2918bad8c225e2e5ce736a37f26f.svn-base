...
int ProbRates::calc(const vector<Ion>& grp, const MeasurementArray& MA, 
  ...
  // store names to be sure we do not get confused later
  ...
  for (int nedb = 41; nedb < 42; nedb++){  // few edbs for debugging
	...
	cout << thisprog << " -D- calc. Nprob for " << grp[s].name << ", "; 
      ...
      for (int tofch = 0; tofch < MAXTOFCH; tofch++){
	for (int esdch = 0; esdch < MAXESDCH; esdch++){
	  Nprob[nedb][s] += MA.getElement(nedb,tofch,esdch)*P[s][tofch][esdch];
	  sum += Nprob[nedb][s];
	}
      }
    ...
    /* method: At each (tofch,esdch), give MA(tofch,esdch) counts to
       species with highest probability
    */
...
vector<float> ProbRates::calcProbCenter(const vector<Ion>& grp, 
					const MeasurementArray& MA, int s, 
					int nedb){
  /*  Description:

        Calculates the probabilistic center of the peak for species s
        at charge step nedb.  Checks to be sure element s of grp is
        the same as element s from getProbRate().
 
  */
  string thisprog = "ProbRates::calcProbCenter";
  ...
  for (int tofch = 0; tofch < MAXTOFCH; tofch++){
	...
	P[s][tofch][esdch] = 0.0;
      ...
      maelement = MA.getElement(nedb,tofch,esdch);

      center[0] += (float) tofch * maelement * P[s][tofch][esdch];
      center[1] += (float) esdch * maelement * P[s][tofch][esdch];
      ProbRateSum +=  maelement * P[s][tofch][esdch];
      
//       center[0] += (float) tofch * getProbRate(nedb,s);
//       center[1] += (float) esdch * getProbRate(nedb,s);
...
vector<vector<float> > ProbRates::calcProbCenter(const vector<Ion>& grp, 
						 const MeasurementArray& MA,
						 int s){
  /*  Description:

        Calculates the probabilistic center of the species s peak for all
        EDBs, using calcProbCenter for specified EDB.
 
  */
  vector<vector<float> > centers;
  
  for (int nedb = FIRSTEDB; nedb < MAXEDB; nedb++){
    centers.push_back(calcProbCenter(grp, MA, s, nedb));
  }
