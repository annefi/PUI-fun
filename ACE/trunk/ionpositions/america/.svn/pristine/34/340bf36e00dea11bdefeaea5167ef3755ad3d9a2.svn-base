...
vector<float> ProbRates::calcProbCenter(const vector<Ion>& grp, int s, 
					int nedb){
  /*  Description:
...
//       center[0] += (float) tofch * P[s][tofch][esdch];
//       center[1] += (float) esdch * P[s][tofch][esdch];
...
vector<vector<float> > ProbRates::calcProbCenter(const vector<Ion>& grp, 
  ...
  */
  ...
  for (int nedb = FIRSTEDB; nedb < MAXEDB; nedb++){
    centers.push_back(calcProbCenter(grp, s, nedb));
  }
