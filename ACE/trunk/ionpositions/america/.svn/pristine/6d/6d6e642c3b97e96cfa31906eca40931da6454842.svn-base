...
/*****************************************************************************
 * $Header: /shrg1/ace/sw/cvsroot/libhef/swig/a3dif_wrap.c,v 1.1.1.1 2000/06/02 13:37:24 jraines Exp $
 *
 * perl5ptr.swg
 *
 ...
 *
 * $Log: a3dif_wrap.c,v $
 * Revision 1.1.1.1  2000/06/02 13:37:24  jraines
 * Imported sources
 *
 * Revision 1.1  1999/02/27 16:14:59  hefti
 * tof
 *
...
#define SWIG_init    boot_a3dif

#define SWIG_name   "a3dif::boot_a3dif"
#define SWIG_varinit "a3dif::var_a3dif_init();"
#ifdef __cplusplus
extern "C"
#endif
#ifndef PERL_OBJECT
SWIGEXPORT(void,boot_a3dif)(CV* cv);
#else
SWIGEXPORT(void,boot_a3dif)(CPerl *, CV *cv);
#endif
...
extern double aval(double *,int );
#ifdef PERL_OBJECT
#define MAGIC_CLASS _wrap_a3dif_var::
class _wrap_a3dif_var : public CPerl {
public:
#else
#define MAGIC_CLASS
#endif
...
XS(_wrap_aval) {
    ...
    cv = cv;
    if ((items < 2) || (items > 2)) 
        croak("Usage: aval(x,index);");
    if (SWIG_GetPtr(ST(0),(void **) &_arg0,"doublePtr")) {
        croak("Type error in argument 1 of aval. Expected doublePtr.");
        XSRETURN(1);
    }
    _arg1 = (int )SvIV(ST(1));
    _result = (double )aval(_arg0,_arg1);
    ST(argvi) = sv_newmortal();
    sv_setnv(ST(argvi++), (double) _result);
    XSRETURN(argvi);
}

XS(_wrap_perl5_a3dif_var_init) {
    dXSARGS;
    SV *sv;
    cv = cv; items = items;
    XSRETURN(1);
...
XS(boot_a3dif) {
	 ...
	 char *file = __FILE__;
	 cv = cv; items = items;
	 newXS("a3dif::var_a3dif_init", _wrap_perl5_a3dif_var_init, file);
	 newXS("a3dif::a3opencyclefile", _wrap_a3opencyclefile, file);
	 newXS("a3dif::a3closecyclefile", _wrap_a3closecyclefile, file);
	 newXS("a3dif::a3readcycle", _wrap_a3readcycle, file);
	 newXS("a3dif::a3expand", _wrap_a3expand, file);
	 newXS("a3dif::a3gcnq", _wrap_a3gcnq, file);
	 newXS("a3dif::a3gcnm", _wrap_a3gcnm, file);
	 newXS("a3dif::aval", _wrap_aval, file);
/*
