...
class dutycycle:
    ...
    # simple gaussian. standardisation has to be done separately!!!
    ...
    # simple kappafunction, standadisation has to be done separately!!!
    def kappaf(self,pos,sigma,kappa):
        "simple kappafunction : returns value of kappa function at 'pos' ; Standardization and shift of the peak position has to be done outside!"
        if ((pos < 5.*sigma) and (pos > -5.*sigma)):
            return (1+(pos**2/(kappa*sigma**2)))**-kappa
        else:
            return 0.
    ...
    def peak1mag(self,x,y,z):
        return self.p1hight*self.kappaf(x,self.vth,self.kappa)*self.kappaf(y,self.vth*self.vthr,self.kappa)*self.kappaf(z,self.vth*self.vthr,self.kappa)

    # Peak2 in MAG kartesian koordinates
    def peak2mag(self,x,y,z):
        return self.p2hight*self.kappaf(x,self.vth,self.kappa)*self.kappaf(y,(self.vth*self.vthr),self.kappa)*self.kappaf(z,(self.vth*self.vthr),self.kappa)
