...
def expand(abedb,adtime,abQAC,pcyc,debug=False):
  """  
  BYTE  *pabdum;
  CPHA  stcpha;
  MPHA  stmpha;
  ...
  for ie in range(NEDB):
    ...
    if ((abQAC[ie]    == 0 ) and (hex(uint8(abedb[ie][0])) == "0xeb" ) and (hex(uint8(abedb[ie][1])) == "0x90") and (hex(uint8(abedb[ie][2])) == "0xda") and (abedb[ie][5] == ie)): # QAC/sync/number 
      ...
      for i in range(pcyc.asxNof[ie]):
        ...
        # RANGE 1,3,5 : Priority range 1, Detextor Id 0,1,2
        # RANGE 0,2 : Priority range 0, Detextor Id 0,1
        # Range 4 : Priority range 0, Detextor Id 2 if ESSD > 0, else Priority range 0, and dummy did val = 7 (not -1 because of uint) 
