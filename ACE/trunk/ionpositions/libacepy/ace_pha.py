    ...
    def load_year(self,year):
        ...
        for doy in range(self.tf[0][0],self.tf[-1][1]+1):
            ...
            for tmprng in self.rng:
                try:
                    dat = loadtxt("/data/etph/ace/lv0/rng%i/%.4i/%.3i.pha"%(tmprng,year,doy),skiprows=2)
            ...
            #try:
            #    dat = loadtxt("/data/etph/ace/lv0/rng2/%.4i/%.3i.pha"%(year,doy),skiprows=2)
            #    tmpmq = tof2mq_new_fulltof(int64(dat[:,3])-1,dat[:,4])
            #    mask = (tmpmq > self.mqrange[0]) * (tmpmq < self.mqrange[1]) 
            #    doys = append(doys,dat[:,2][mask])
            #    epq = append(epq,int64(dat[:,3])[mask])
            ...
            #    mq = append(mq,tmpmq[mask])
            #except:
            #    print "No rng2 data for doy ",doy,year
        ...
        self.data["year"] = append(self.data["year"],ones(doys.shape)*year)
        ...
        if self.swprops:
            ...
            sweswi.bin_data(append(unique(doys),unique(doys)[-1]+1./24./5.),unique(doys),prod1="time",prod2="d00")
        ...
        if self.mag:
            ...
            mag.transform_data(append(unique(doys),unique(doys)[-1]+1./24./5.),prodt="time")
            print "Start syncing MAG data",year
            mag.bin_data(append(unique(doys),unique(doys)[-1]+1./24./5.),unique(doys),prod1="time",prod2="d00")
