...
def filterstr(s):
    """ Filters strings so that it will not be mangled by GNUplot. """
...
class Gnuplot:
    ...
    Methods:
        ...
        'xlabel', 'ylabel', 'zlabel', 'cblabel', 'title' -- set
            corresponding plot attribute.
    ...
    def __init__(self, filename=None, persist=None, debug=0, noraise=True, sigintable=True):
        ...
        if filename is None:
            self.gnuplot = gp.GnuplotProcess(persist=persist, noraise=noraise)
        else:
        ...
        # try to enable mouse coordinates (remember to look also at your .gnuplot
        # configuration file)
        if gp.GnuplotOpts.default_term=="x11":
            self('set mouse')
        ...
        if sigintable:
            ...
            self.old_sigint=signal(SIGINT, self.cancel)

        from objects import GPItemsManager
        self.items=GPItemsManager(self)

    def addItem(self, item):
        """ Shortcut for self.items.add(...) """
        self.items.add(item)

    def removeItem(self, item):
        """ Shortcut for self.items.remove(...) """
        self.items.remove(item)

    def removeAllItems(self):
        """ Shortcut for self.items.clear(...) """
        self.items.clear()
    ...
    def _remove_last_from_queue(self):
    ...
    def saveState(self, target):
        """ Save the whole state of GNUplot, including all data files
        into the directory named by target. Files will be renamed and will have generic file names
        <target>/dataX.dat
        The GNUplot script will be saved as <target>/plot.gpt.

        FIXME:THIS IS STILL INCOMPLETE AND SHOULD NOT YET BE USED!
        """
        from helpers import MakeSureDirExists
        self.refresh()
        MakeSureDirExists(gpvars["STATE_DIRECTORY"]+"/"+folder)
        # FIXME: Test for FIFO plots etc.
    ...
    def setXrange(self, value):
        """ set_range for the x range. """
        self.set_range("xrange", value)
    def setX2range(self, value):
        """ set_range for the x2 range. """
        self.set_range("x2range", value)
    def setYrange(self, value):
        """ set_range for the y range. """
        self.set_range("yrange", value)
    def setY2range(self, value):
        """ set_range for the y2 range. """
        self.set_range("y2range", value)
    def setZrange(self, value):
        """ set_range for the z range. """
        self.set_range("zrange", value)
    def setCBrange(self, value):
        """ set_range for the color bar range. """
        self.set_range("cbrange", value)
    ...
    def x2label(self, s=None, offset=None, font=None):
        """Set the plot's x2label."""
        self.set_label('x2label', s, offset=offset, font=font)

    def ylabel(self, s=None, offset=None, font=None):
        """Set the plot's ylabel."""
        self.set_label('ylabel', s, offset=offset, font=font)
    def y2label(self, s=None, offset=None, font=None):
        """Set the plot's y2label."""
        self.set_label('y2label', s, offset=offset, font=font)

    def zlabel(self, s=None, offset=None, font=None):
        """Set the plot's zlabel."""
        self.set_label('zlabel', s, offset=offset, font=font)

    def cblabel(self, s=None, offset=None, font=None):
        """Set the plot's colour bar label."""
        self.set_label('cblabel', s, offset=offset, font=font)

    def title(self, s=None, offset=None, font=None):
        """Set the plot's title."""
        self.set_label('title', s, offset=offset, font=font)

    def log(self, axis="y", yeah=True):
        if yeah:
            self("set logscale %s" % axis)
        else:
            self("set nologscale %s" % axis)
            
    def nolog(self, axis="y"):
        self.log(axis, False)
    ...
    def intoCollection(self, fnbase="", folder=""):
        """ Makes a few default plots with hopefully sensible default options
        and puts them into the collection as specified by the variable
        PLOT_DIRECTORY in ~/.gnuplot. Switches to plotting mode using the
        SWITCH_HARDCOPY string and after the plots, back to display mode using
        SWITCH_INTERACTIVE.
        fnbase: specifies the base string of the file name (without any extension!)
        folder: Specifies an optional sub(sub...)-folder to plots in (for more
        specific organization of the PLOT_DIRECTORY).
        """
        ...
        from variables import gpvars
        self(gpvars["SWITCH_HARDCOPY"])
        from helpers import MakeSureDirExists
        MakeSureDirExists(gpvars["PLOT_DIRECTORY"]+"/"+folder)
        fnbase=fnSimplify(gpvars["PLOT_DIRECTORY"]+"/"+folder+"/"+fnbase)
        self.hardcopy(fnbase+"_print.eps", color=True,
                      enhanced=True, fontsize=10,
                      fontname="Times-Roman", eps=True,
                      solid=True)
        self.hardcopy(fnbase+"_print.ps", color=True,
                      enhanced=True, fontsize=10,
                      fontname="Times-Roman",
                      solid=True, mode="landscape")
        self.hardcopy(fnbase+"_print_bw.eps", color=False,
                      enhanced=True, fontsize=10,
                      fontname="Times-Roman", eps=True,
                      solid=True)
        self.hardcopy(fnbase+"_present16.eps", color=True,
                      enhanced=True, fontsize=16,
                      fontname="Times-Roman", eps=True,
                      solid=True)
        self.hardcopy(fnbase+"_present20.eps", color=True,
                      enhanced=True, fontsize=20,
                      fontname="Times-Roman", eps=True,
                      solid=True)
        self.hardcopy(fnbase+".png", terminal="png")
        self(gpvars["SWITCH_INTERACTIVE"])


    # This is adapted from the GNUplot bidirectional example code,
    # which can be found at
    # http://www.physik.tu-dresden.de/~baecker/python/GnuplotBiDir.py
    def eval(self, var, ass=float):
        """Evals a print expression in gnuplot and passes the result to
        python. Type describes the output conversion to be used, the default
        is a float.

        Example usage:
        gp=Gnuplot()
        gp.eval(\"pi\") --> 3.1415...
        gp.eval(\"defined(pi)\") --> 1.0
        """

        self(" set print \"-\"\n")      # print output to stdout
        #        self("if (defined(%s)) print %s ; else print \"None\" \n" % (var,var))
        self("if (defined(%s) && (%s+1!=%s)) print %s ; else print \"None\" \n" % (var,var,var,var))
        result=self.gnuplot.backchannel.readline()[:-1]
    ...
    def get_range(self, axis):
        return (self.eval("GPVAL_%s_MIN" % axis.upper()),
                self.eval("GPVAL_%s_MAX" % axis.upper()))

    def getXrange(self):
        return self.get_range("x")

    def getYrange(self):
        return self.get_range("y")

    def sync(self):
        """ Synchronizes the GNUplot instance with the python program,
        if it is busy processing a plot or other actions.
	"""
        from sys import stderr
        # Using eval() is better than any try at this with fsync() etc., as it
        # does not rely on GNUplot being single-threaded.
#        print>>stderr, ">>> GNUPlot",
        stderr.flush()
        self.eval("pi")
#        print>>stderr, "synchronized."

    def getxy(self):
        """ Returns an X/Y coordinate pair which has to be selected
        with a click of the mouse. """
        self("pause mouse")
        self("\n")
        return (self.eval("MOUSE_X"),
                self.eval("MOUSE_Y"))

    def cancel(self, a=None, b=None):
        from os import kill
        from signal import SIGINT
        kill(self.gnuplot.gpproc.pid, SIGINT)
        if (a!=None) or (b!=None): # only pass through if called by signal
            self.old_sigint(a,b)

    def labelTics(self, axis, tics):
        s="set %stics (" % axis
        for t in tics:
            s+='"%s" %f, ' % t
        s=s[:-2]
    ...
    def term(self, terminal):
        """ Short-cut for setting the output terminal of the gnuplot instance.
        The possible values are at the moment:
        null: no graphical output at all
        x11: usual X11 on-screen output."""
        if terminal=="null":
            self("set terminal post")
            self("set output '/dev/null'")
        elif terminal=="x11":
            self("set terminal x11")
        else:
            raise Exception, "Invalid terminal to set through Gnuplot::term(self, terminal)."
        
    def labelXTics(self, tics):
        self.labelTics("x", tics)
    def labelX2Tics(self, tics):
        self.labelTics("x2", tics)
    def labelYTics(self, tics):
        self.labelTics("y", tics)
    def labelY2Tics(self, tics):
        self.labelTics("y2", tics)
