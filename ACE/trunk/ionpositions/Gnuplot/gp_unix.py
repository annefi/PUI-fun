...
class GnuplotProcess:
    ...
    def __init__(self, persist=None, noraise=True):
        ...
        if persist:
            ...
            self.gpproc=popen2.Popen3(('%s -persist '+otheropts) % GnuplotOpts.gnuplot_command)
            
        else:
            self.gpproc=popen2.Popen3(('%s '+otheropts) % GnuplotOpts.gnuplot_command)

        self.gnuplot, self.backchannel = self.gpproc.tochild, self.gpproc.fromchild
