...
class ace_sweswi(dbData):
    ...
    def bin_data(self,t1,t2,year,prod1="d00",prod2="time"):
        """
        ...
        t1 -> d00 already with 
        """
        # Mean Solar wind speed and Standard deviation within new time bins 
        ...
        nrT[nrT==0.] = 1.
        mvsw,x = histogram(pT,t1,weights=vsw)
        mvsw = mvsw/nrT
        svsw,x = histogram(pT,t1,weights=vsw**2)
        svsw = sqrt(svsw/nrT-mvsw**2)
        self.remove_submask("Master","vsw")

        # Mean Solar wind density and Standard deviation within new time bins 
        self.set_mask("Master","dsw",0.,op="gt",reset=True)
        pT = self.get_data("Master",prod1)+1./24./60.
        dsw = self.get_data("Master","dsw")
        nrT,x= histogram(pT,t1)
        mdsw,x= histogram(pT,t1,weights=dsw)
        nrT[nrT==0.] = 1.
        mdsw = mdsw/nrT
        sdsw,x = histogram(pT,t1,weights=dsw**2)
        sdsw = sqrt(sdsw/nrT-mdsw**2)
        self.remove_submask("Master","dsw")

        # Mean Solar wind temperature and Standard deviation within new time bins 
        self.set_mask("Master","tsw",0.,op="gt",reset=True)
        pT = self.get_data("Master",prod1)+1./24./60.
        tsw = self.get_data("Master","tsw")
        nrT,x = histogram(pT,t1)
        mtsw,x=histogram(pT,t1,weights=tsw)
        nrT[nrT==0.] = 1.
        mtsw = mtsw/nrT
        stsw,x = histogram(pT,t1,weights=tsw**2)
        stsw = sqrt(stsw/nrT-mtsw**2)
        self.remove_submask("Master","tsw")

        # Mean He/H ratio and Standard deviation within new time bins 
        self.set_mask("Master","heh",0.,op="gt",reset=True)
        pT=self.get_data("Master",prod1)+1./24./60.
        heh = self.get_data("Master","heh")
        nrT,x = histogram(pT,t1)
        mheh,x = histogram(pT,t1,weights=heh)
        nrT[nrT==0.]=1.
        mheh = mheh/nrT
        sheh,x = histogram(pT,t1,weights=heh**2)
        sheh = sqrt(sheh/nrT-mheh**2)
