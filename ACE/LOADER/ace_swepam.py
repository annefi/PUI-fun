...
class ace_swepam(dbData):
    def load_data(self,*args,**kwargs):
        ...
        if kwargs.has_key("path"):
            ...
            if self.fmt=="new":
                self.path="/data/ivar/ace/swepam/12mindata/"
            else:
                self.path="/data/etph/ace/swepam/"
        if kwargs.has_key("merged"):
        ...
        if self.fmt=="old":
            ...
            for year in self.year:
                fin=open(self.path+"64s"+str(year)+".dat","r")
    ...
    def bin_data(self,t1,t2,prod1="d00",prod2="time"):
        ...
        indT=searchsorted(t1,self.get_data("Master",prod1))-1
        indT=indT[(indT>=0)*(indT<t1.shape[0]-1)]
        nrT,x=histogram(self.get_data("Master",prod1),t1)
        mask=nrT[indT]>0
        mvsw,x=histogram(self.get_data("Master",prod1)[mask],t1,weights=self.get_data("Master","vsw")[mask])
        mvsw/=nrT
        svsw,x=histogram(self.get_data("Master",prod1)[mask],t1,weights=(self.get_data("Master","vsw")[mask]-mvsw[indT][mask])**2)
        svsw=sqrt(svsw/nrT)
        ...
        nrT,x=histogram(self.get_data("Master",prod1),t1)
        mask=nrT[indT]>0
        mdsw,x=histogram(self.get_data("Master",prod1)[mask],t1,weights=self.get_data("Master","dsw")[mask])
        mdsw/=nrT
        sdsw,x=histogram(self.get_data("Master",prod1)[mask],t1,weights=(self.get_data("Master","dsw")[mask]-mdsw[indT][mask])**2)
        sdsw=sqrt(sdsw/nrT)
        ...
        nrT,x=histogram(self.get_data("Master",prod1),t1)
        mask=nrT[indT]>0
        mtsw,x=histogram(self.get_data("Master",prod1)[mask],t1,weights=self.get_data("Master","tsw")[mask])
        mtsw/=nrT
        stsw,x=histogram(self.get_data("Master",prod1)[mask],t1,weights=(self.get_data("Master","tsw")[mask]-mtsw[indT][mask])**2)
        stsw=sqrt(stsw/nrT)
        ...
        nrT,x=histogram(self.get_data("Master",prod1),t1)
        mask=nrT[indT]>0
        mheh,x=histogram(self.get_data("Master",prod1)[mask],t1,weights=self.get_data("Master","heh")[mask])
        mheh/=nrT
        sheh,x=histogram(self.get_data("Master",prod1)[mask],t1,weights=(self.get_data("Master","heh")[mask]-mheh[indT][mask])**2)
        sheh=sqrt(sheh/nrT)
    ...
    def save_bin(self,path="/data/ivar/ace/swepam/12mindata/"):
        for year in self.year:
            self.reset_mask("Master")
