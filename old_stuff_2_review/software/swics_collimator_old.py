...
class collimator(object):
    def __init__(self, dang_para=1., dang_perp=1., aspphi=-20.000001, asptheta=20.000001, vel=600., vsw=300,
                 calc_full=False):
        """
        Class to calculate the field of view of SWICS (ACE and later on Ulysses)
        dang_para,dang_perp = Step width in degrees to calc entrance slit in parallel and perpendicular direction.
        Total opening angle parallel is 90 degrees, and 2 degrees perpendicular respectively
        aspphi,ansptheta = Aspect angle in degrees, i.e. orientation of the spacecraft spin axis. NOTE rotation
        around theta is against usual sense of direction therefore the rotaion has to be by -asptheta!!!!
        Coordinates (GSE):
    ...
    def _calc_points(self):
        ...
        if self.calc_full:
            ...
            for i, ang in enumerate(self.ang_para[self.md1]):
                self.det1[:, i * self.ang_perp.shape[0]:(i + 1) * self.ang_perp.shape[0]] = rotate(self.base_points,
                                                                                                   rax, ang,
                                                                                                   deg=True).reshape(
                    self.base_points.shape)
            for i, ang in enumerate(self.ang_para[self.md2]):
                self.det2[:, i * self.ang_perp.shape[0]:(i + 1) * self.ang_perp.shape[0]] = rotate(self.base_points,
                                                                                                   rax, ang,
                                                                                                   deg=True).reshape(
                    self.base_points.shape)
            for i, ang in enumerate(self.ang_para[self.md3]):
                self.det3[:, i * self.ang_perp.shape[0]:(i + 1) * self.ang_perp.shape[0]] = rotate(self.base_points,
                                                                                                   rax, ang,
                                                                                                   deg=True).reshape(
                    self.base_points.shape)
        else:
    ...
    def _calc_FoV(self):
        if self.calc_full:
            ...
            for sec in range(8):
                for ang in angsteps:
                    ind = ang / 3
                    self.FoV[0, sec, :, ind * 150:(ind + 1) * 150] = rotate(self.det1, self.rax, sec * 45. + ang,
                                                                            deg=True).reshape(self.det1.shape)
                    self.FoV[1, sec, :, ind * 150:(ind + 1) * 150] = rotate(self.det2, self.rax, sec * 45. + ang,
                                                                            deg=True).reshape(self.det2.shape)
                    self.FoV[2, sec, :, ind * 150:(ind + 1) * 150] = rotate(self.det3, self.rax, sec * 45. + ang,
    ...
    def plot_vspace(self, ax=None, reduced=False):
        if ax == None:
            ...
            ax.set_xlabel("v$_{sc,x}$ [km/s]", labelpad=12)
            ax.set_ylabel("v$_{sc,y}$ [km/s]", labelpad=12)
            ax.set_zlabel("v$_{sc,z}$ [km/s]", labelpad=12)
        rotax = zeros((3, 201))
        xax = zeros((3, 201))
        ...
        ax.plot(-rotax[0, :], -rotax[1, :], rotax[2, :], ".-", color="gray")
        ...
        ax.set_zticklabels([800, "", 400, "", 0, "", -400, "", -800])
        return ax
