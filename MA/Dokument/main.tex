...
\textbf{Context:}
\\
Pickup ions (PUI) are former neutral atoms that are ionized (by a b or c) in the heliosphere. The neutral atom's origin is either the interstellar medium (interstellar PUI) or a source within the heliosphere (inner-source PUI).\\
...
The details of involved processes like PAS and C. are not completely understood to this day.
A study of the evolution of the VDF helps us to understand the origin of PUI and interplanetary transport effects.\\


While most observation of PUI are limited to 1D reduced velocity measurement in the spacecraft frame, crucial informations about the three-dimensional VDA are lost.  


Most observations of PUI are limited to 1D reduced velocity measurements in the spacecraft frame, which are not sufficient to represent the three dimensional evolution of the PUIs VDF.



-- The aim is to resolve the VDF in three dimensions and to transform it into the SW frame. 
which then allows to \\ 
.
\\ \\
\textbf{Methods:}\\
In this Master thesis we analyze He+ PUI measured with the Solar Wind Ion Composition Spectrometer (SWICS) onboard the spacecraft Ulysses. SWICS is a time-of-flight mass spectrometer, that is able to determine the mass, mass-per-charge and energy of ions by independent measurements of the ion's energy-per-charge, time-of-flight and residual energy. Additionally, SWICS provides directional resolution of incident ions by a three-part division of the sensor and a sectorization of Ulysses' spin. 
With this directional information a transformation from the measured absolute velocity into three dimensional velocity components can be performed.
\\
...
In the third chapter a virtual detector is introduced and utilized to translate the He+ Pulse Height Analysis data into three dimensional velocity components.
To achieve this translation Ulysses' orientation in space and its eigen-velocity have been considered.
