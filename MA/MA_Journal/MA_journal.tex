...
\newcommand{\DayInMar}[3][]{\vspace{2cm}%
	\noindent \tikz \draw [draw=black, ultra thick, #1]
	($(current page.center)-(0.5\linewidth,0)$) -- 
	($(current page.center)+(0.5\linewidth,0)$)
	node [midway, fill=white] {\textbf{#2 -- #3. März}};
}
...
%
\DayInFeb{Di}{16}
ulysses\_traj\_spice.py geschrieben. Wo soll ich den AA berechnen? Hier oder direkt dort, wo die Trajektorie weg geschrieben wird (in spice\_ulysses.py)?\\
Wie berechne ich den AA nun richtig? Auf welchem System Erde/Ulysses (1950,2000,HC,HG)? Gibt es eine SPICE-interne Funktion um den Winkel zu verifizieren? $\Rightarrow$ spkpos: Vektor Ulysses--Erde; dann Vektor in RTN umrechnen und von dort in die zwei Komponenten\\
$\Rightarrow$ Verworfen.

\DayInFeb{Mi}{24}
DbData für Python3 geladen. Dafür pylib3 installiert: In /usr/bin/ \textit{./pip3 install pylib3}. Dann im Zielordner \textit{svn co svn+ssh://asterix/data/subversion/etph/pylib3}\\ \\
Synchronisierung mit Spice Trajektroie läuft. ToDo:
\begin{itemize}
	\item dbData3 einarbeiten und testen DONE
	\item Geschwindigkeiten einpflegen DONE (nach langer Fehlersuche)
	\item flachen Winkel mit in die Trajektoriendatei? Oder irgendwo kurze Berechnungsfunktion. Quasi nur für den nächsten Punkt...
	\item Vergleich mit alten Werten
	\item Koordinatenskripte mergen DONE
	\item Journal polishen: ToDo Package und Datum...
	\item Koordinatenkram aufschreiben: Graphik der Koordinatensysteme (Neigung der Erd- und Sonnenachse?!), Ulysses Trajektorie Anwendung?
	\item in calc\_d90 Division checken: python3-kompatibel? Schaltjahr...?
	\item SPICE Skript überarbeiten
	\item Koordinatenskripte überarbeiten: etCoords ersetzen
\end{itemize}

\DayInMar{Di}{23}
ulysses\_SPICE überarbeiten. Pool-Variante fertig. Als nächstes SPICE einpflegen. Koordinatenskripte dafür checken: Nur Johans? Eigenes? etCoords ersetzen!

\DayInMar{Mi}{24}
Werde Johans Funktionen übernehmen, wenn ich an die Stelle(n) komme. LocateUlysses funktioniert. Als nächstes: Plotfunktion einbauen, compare, write... dann überlegen, wie ich manuelle Rotationen implementiere.

\DayInMar{Do}{25}
Nächster Schritt: Farbgebung und Labels in Plots schön machen
