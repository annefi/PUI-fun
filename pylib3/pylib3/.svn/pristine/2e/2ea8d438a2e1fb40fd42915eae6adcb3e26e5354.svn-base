...
class ShowMask(object):
    ...
    def _get_cdata(self, prod, key):
        ...
        """
        if prod not in self.db.data.keys():
            cdata = np.ones(10, dtype=str)
            cdata[9] = "User-Defined Submask: '%s'" % (prod)
            Nevents = np.sum(self.db.mask[self.cmask].get())
        elif prod == "appliedmasks":
            cdata = list(self.db.mask[key].appliedmasks)
            Nevents = np.sum(self.db.mask[self.cmask].get())
        elif prod == "directmasks":
            cdata = list(self.db.mask[key].directmasks.keys())
            Nevents = np.sum(self.db.mask[self.cmask].get())
            ...
            if key != "Master":
                ...
                cdata = self.db.data[prod]
                Nevents = len(self.db.data[list(self.db.data.keys())[0]])
    ...
    def compute(self):
        ...
        for kindex, key in enumerate(masks):
            ...
            products = list(self.db.mask[key].submasks.keys())
            if len(self.db.mask[key].appliedmasks) > 0:
                products.append("appliedmasks")
            if len(list(self.db.mask[key].directmasks.keys())) > 0:
                products.append("directmasks")
            if len(products) == 0 and key == "Master":
                self.Nevents = float(
                    self.db.data[list(self.db.data.keys())[0]].shape[0])
